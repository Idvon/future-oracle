...
3. **Нормализатор** — превращает сырые RSS-записи в сигналы `news_mention_recent`
   (значение свежести 0.9 / 0.6 / 0.3).
4. **Пересчёт** — генерирует свежий прогноз для каждого активного события.
...
| Steam Store (демо) | store | 0.8 | предзаказы |
...
После целевой даты оператор разрешает событие (или это можно делать автоматически
по проверке в магазине/Wikipedia):
...
Brier означает, что модель хорошо откалибрована.
...
Этот прототип не использует LLM. Правила, веса и скоринг детерминированы и полностью
объяснимы. Если позже добавите LLM (например, для классификации фазы разработки по
новостям), опишите здесь, как сохраняются факты и какие части остаются на правилах.
...
├── collectors/             # wikipedia_collector.py, rss_collector.py
...
- Авторазрешение событий: сравнение целевой даты с наличием игры в Wikipedia/Steam Store.
